...
//indo pra pagina carnes
$app->get(
    '/carnes',
    function () {
        
        require_once("view/carnes.php");
        
    }
);
//indo pra pagina alimentos
$app->get(
    '/alimentos',
    function () {
        
        require_once("view/alimentos.php");
...
//indo pra pagina limpeza
$app->get(
    '/limpeza',
    function () {
        
        require_once("view/limpeza.php");
...
$app->get(
    ...
    function () {
        ...
        require_once("view/promocao.php");
...
//indo pra exibir produto ao escolher
$app->get('/produtos', function(){

    $sql = new Sql();

    $data = $sql->select("SELECT * FROM tb_produtos where preco_promorcional > 16 order by preco_promorcional  limit 8;");
...
$app->get('/carness', function(){

    $sql = new Sql();

    $data = $sql->select("
        SELECT  * from tb_produtos
        where id_prod>0 and id_prod<10
        LIMIT 8;
    ");
...
$app->get('/limpezaa', function(){

    $sql = new Sql();

    $data = $sql->select("
        SELECT  * from tb_produtos
        where id_prod>20 and id_prod<=30
        LIMIT 8;
    ");
...
$app->get('/promocaoo', function(){

    $sql = new Sql();

    $data = $sql->select("
        SELECT  * from tb_produtos
        where id_prod>30 and id_prod<40
        LIMIT 8;
    ");
    ...
    foreach ($data as &$produto) {
        $preco = $produto['preco'];
        $centavos = explode(".", $preco);
        $produto['preco'] = number_format($preco*1.2, 2, ",", ".");
        $produto['centavos'] = end($centavos);
        $produto['parcelas'] = 4;
        $produto['parcela'] = number_format($preco/$produto['parcelas'], 2, ",", ".");
        $produto['total'] = number_format($preco, 2, ",", ".");
...
$app->get('/alimentoss', function(){

    $sql = new Sql();

    $data = $sql->select("
        SELECT  * from tb_produtos
        where id_prod>10 and id_prod<20
        LIMIT 8;
    ");
